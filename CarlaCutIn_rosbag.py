...
class CarlaLongitudinalControl(object):
    ...
    def __init__(self):
        ...
        self.heroGoal.pose.append(self.ego_pose)
    ...
    def spawn_hero(self):
        ...
        # Get vehicle blueprint.
        self.actor_filter = "vehicle.mustang.mustang"
        blueprint = secure_random.choice(
            self.world.get_blueprint_library().filter(self.actor_filter))
        blueprint.set_attribute('role_name', "{}".format(self.role_name))
        if blueprint.has_attribute('color'):
    ...
    def hero_control(self):
        self.player.set_target_velocity(carla.Vector3D(x=self.hero_vel_x[self.count_hero],
                                                       y=-self.hero_vel_y[self.count_hero],
                                                       z=0.0))
        ...
        self.heroGoal.pose[1].position.x = self.ego_pos_x[self.count_hero]
        self.heroGoal.pose[1].position.y = self.ego_pos_y[self.count_hero]
    ...
    def run(self):
        ...
        for topic, msg, t in bag.read_messages(topics=['/gazebo/model_states/']):
            if len(msg.twist) > 2:
                ...
                self.ego_pos_x.append(msg.pose[1].position.x)
                self.ego_pos_y.append(msg.pose[1].position.y)
                ...
                self.hero_yaw.append(msg.pose[2].orientation.z)
                ...
                self.hero_yaw.append(0.0)
                self.hero_time.append(t.to_sec())
            elif len(msg.twist) > 0:
                self.vel_bag_time.append(t.to_sec())
                self.vel_goal.append(0.0)
                self.ego_pos_x.append(0.0)
                self.ego_pos_y.append(0.0)
                self.hero_vel_x.append(0.0)
                self.hero_vel_y.append(0.0)
                self.hero_pos_x.append(0.0)
                self.hero_pos_y.append(0.0)
